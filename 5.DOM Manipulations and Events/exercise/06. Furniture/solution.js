function solve() {
  ...
  const [generateInput, buyOutput] = Array.from(document.querySelectorAll('textarea'))
  ...
  function generate() {
    const data = JSON.parse(generateInput.value);
  ...
  function buy() {
    ...
    buyOutput.textContent += `Bought furniture: ${bought.join(', ')}\r\n`;
    buyOutput.textContent += `Total price: ${price.toFixed(2)}\r\n`;
    buyOutput.textContent += `Average decoration factor: ${decoration / checkBoxes.length}`;
  }
}